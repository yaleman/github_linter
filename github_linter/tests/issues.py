...
def check_open_issues(
    ...
    if repo.open_issues:
        add_result(warnings_object, CATEGORY, f"There are {repo.open_issues} open issues for {repo.full_name}")
...
def check_open_prs(
    ...
    if pulls.totalCount:
        logger.debug("There's {} PRs... listing at least the latest 10.", pulls.totalCount)
